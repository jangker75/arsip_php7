...
use Session;
	...
	class AdminReportController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Client Id","name"=>"client_id","join"=>"client,nama"];
			$this->col[] = ["label"=>"Unit Kerja Id","name"=>"unit_kerja_id","join"=>"unit_kerja,nama"];
			$this->col[] = ["label"=>"Lokasi Vault Id","name"=>"lokasi_vault_id","join"=>"lokasi_vault,nama"];
			$this->col[] = ["label"=>"Jenis Dok Id","name"=>"jenis_dok_id","join"=>"jenis_dok,id"];
			$this->col[] = ["label"=>"Status Id","name"=>"status_id","join"=>"status,nama"];
			$this->col[] = ["label"=>"Status Approve","name"=>"status_approve"];
			# END COLUMNS DO NOT REMOVE THIS LINE

			# START FORM DO NOT REMOVE THIS LINE
			$this->form = [];
			$this->form[] = ['label'=>'Cabang Id','name'=>'cabang_id','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'cabang,nama'];
			$this->form[] = ['label'=>'Client Id','name'=>'client_id','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'client,nama'];
			$this->form[] = ['label'=>'Unit Kerja Id','name'=>'unit_kerja_id','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'unit_kerja,nama'];
			$this->form[] = ['label'=>'Lokasi Vault Id','name'=>'lokasi_vault_id','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'lokasi_vault,nama'];
			$this->form[] = ['label'=>'Jenis Dok Id','name'=>'jenis_dok_id','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'jenis_dok,id'];
			$this->form[] = ['label'=>'Status Id','name'=>'status_id','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'status,nama'];
			...
			$this->form[] = ['label'=>'File Atc','name'=>'file_atc','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Tgl Input','name'=>'tgl_input','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Tgl Pemindahan','name'=>'tgl_pemindahan','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
	    ...
	    */
		...
		public function getIndex() {
			//First, Add an auth
			 ...
			 $data['result'] = DB::table('box')->orderby('id','desc')->paginate(10);
