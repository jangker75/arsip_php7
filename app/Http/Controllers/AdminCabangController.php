...
use App\Models\Cabang;
	...
	class AdminCabangController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = [
				"label" => "Total Image", "name" => "id", "callback" => function ($row) {
					$cabang = Cabang::find($row->id);
					return $cabang->attImage->count();
				},
	        ...
	        */
			...
			$this->addaction[] = ['label' =>'Upload Foto','url' => CRUDBooster::mainpath('upload-foto/[id]'),
			'icon'=>'fa fa-image','color'=>'success'];
	    ...
	    */
		...
		public function getUploadFoto($id)
		{
			$data['page_title'] = "Upload Foto";
			$data['page']		= 'Cabang';
			$data['cabang'] = Cabang::find($id);
			return $this->view('upload_foto', $data);
		}
		public function postUploadFoto(Request $req)
		{
			// dd($req->all());
			$idCabang = request("id");
			// $cabang = DB::table('cabang')->where('id', $idCabang)->first();
			$cabang = Cabang::find($idCabang);
			if (count($cabang->attImage) > 0) {
				foreach ($cabang->attImage as $media) {
					if (!in_array($media->file_name, $req->input('photo', []))) {
						$media->delete();
					}
				}
			}
			$media = $cabang->attImage->pluck('file_name')->toArray();

			foreach ($req->input('photo', []) as $file) {
				if (count($media) === 0 || !in_array($file, $media)) {
					$cabang->addMedia(storage_path('tmp/uploads/' . $file))->toMediaCollection('photo_cabang');
				}
			}
			return CRUDBooster::redirect(CRUDBOOSTER::mainpath(),"Upload Foto berhasil !","success");
		}
	}